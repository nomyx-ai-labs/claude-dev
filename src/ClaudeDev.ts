...
export class ClaudeDev {
    ...
    private automaticallyRunTasks: boolean
    ...
    private diffViewFocused = false
    private providerName: string = "Claude Dev"
    ...
    constructor(
        ...
        this.providerName = (apiConfiguration.apiProvider as string) || ""
        this.maxRequestsPerTask = maxRequestsPerTask ?? DEFAULT_MAX_REQUESTS_PER_TASK
        this.customInstructions = customInstructions
        this.alwaysAllowReadOnly = alwaysAllowReadOnly ?? false
        this.automaticallyRunTasks = automaticallyRunTasks ?? false

        // Add event listener for diff view focus
        vscode.window.onDidChangeTextEditorSelection(this.handleDiffViewFocus.bind(this))
    ...
    private handleDiffViewFocus(event: vscode.TextEditorSelectionChangeEvent) {
        if (event.textEditor.document.uri.scheme === 'file' &&
            event.textEditor.document.fileName.includes('claude-dev-')) {
            this.diffViewFocused = true
        }
    }
    ...
    updateAutomaticallyRunTasks(automaticallyRunTasks: boolean | undefined) {
        this.automaticallyRunTasks = automaticallyRunTasks ?? false
    }
    ...
    async executeTool(toolName: ToolName, toolInput: any): Promise<ToolResponse> {
        switch (toolName) {
            case "write_to_file":
                return this.automaticallyRunTasks ?
                    this.writeToFile(toolInput.path, toolInput.content) :
                    this.writeToFileWithConfirmation(toolInput.path, toolInput.content)
            case "read_file":
                return this.automaticallyRunTasks ?
                    this.readFile(toolInput.path) :
                    this.readFileWithConfirmation(toolInput.path)
            case "list_files":
                return this.automaticallyRunTasks ?
                    this.listFiles(toolInput.path, toolInput.recursive) :
                    this.listFilesWithConfirmation(toolInput.path, toolInput.recursive)
            case "list_code_definition_names":
                return this.automaticallyRunTasks ?
                    this.listCodeDefinitionNames(toolInput.path) :
                    this.listCodeDefinitionNamesWithConfirmation(toolInput.path)
            case "search_files":
                return this.automaticallyRunTasks ?
                    this.searchFiles(toolInput.path, toolInput.regex, toolInput.filePattern) :
                    this.searchFilesWithConfirmation(toolInput.path, toolInput.regex, toolInput.filePattern)
            case "execute_command":
                return this.automaticallyRunTasks ?
                    this.executeCommand(toolInput.command) :
                    this.executeCommandWithConfirmation(toolInput.command)
            ...
            case "get_debug_console":
                return this.getDebugConsole()
            case "get_terminal":
            ...
            case "get_page_content":
                return this.getPageContent(toolInput.url)
            case "start_debugging":
                return this.startDebugging(toolInput.config)
            case "stop_debugging":
                return this.stopDebugging()
            case "pause_debugging":
                return this.pauseDebugging()
            case "resume_debugging":
                return this.resumeDebugging()
            case "step_over":
            ...
            case "add_breakpoint":
                return this.addBreakpoint(toolInput.filePath, toolInput.lineNumber)
            case "remove_breakpoint":
                return this.removeBreakpoint(toolInput.filePath, toolInput.lineNumber)
            case "evaluate_expression":
                return this.evaluateExpression(toolInput.expression)
    ...
    getProblemsList(): ToolResponse {
        const problems = vscode.languages.getDiagnostics()
        const problemList: string[] = []
        for (const [, diagnostics] of problems) {
            for (const diagnostic of diagnostics) {
                problemList.push(
                    `${diagnostic.severity === vscode.DiagnosticSeverity.Error ? "Error" : "Warning"}: ${diagnostic.message
                    }`
                )
            }
        }
        if (problemList.length === 0) {
            return "No problems found in the current workspace."
        }
        return problemList.join("\n")
    }

    getDebugConsole(): ToolResponse {
        return (vscode.debug.activeDebugConsole as any)?.name ?? "No active debug console"
    }

    getTerminal(): ToolResponse {
        return vscode.window.activeTerminal?.name ?? "No active terminal"
    }

    getEditor(): ToolResponse {
    ...
    async searchGoogle(query: string): Promise<ToolResponse> {
        // use the google search API to search google - use the GOOGLE_API_KEY and GOOGLE_CX_ID environment variables
        const googleApiKey = process.env.GOOGLE_API_KEY
        const googleCxId = process.env.GOOGLE_CX_ID
        if (!googleApiKey || !googleCxId) {
            return "Error: Missing GOOGLE_API_KEY or GOOGLE_CX_ID environment variables."
        }

        const url = `https://www.googleapis.com/customsearch/v1?key=${googleApiKey}&cx=${googleCxId}&q=${query}`
        // use axios to make the request
        const response = await axios.get(url)
        // return the first ten search results
        const searchResults = response.data.items.slice(0, 10).map((item: any) => item.title + ": " + item.link)
        return searchResults
    }

    async getPageContent(url: string): Promise<ToolResponse> {
        const response = await axios.get(url)
    ...
    async startDebugging(config: vscode.DebugConfiguration): Promise<ToolResponse> {
        const ret = await debuggerController.startDebugging(config)
        return ret ? "Debugging started" : "Failed to start debugging"
    }

    async stopDebugging(): Promise<ToolResponse> {
        await debuggerController.stopDebugging()
        return "Debugging stopped"
    }

    async pauseDebugging(): Promise<ToolResponse> {
        debuggerController.pauseDebugging()
        return "Debugging paused"
    }

    async resumeDebugging(): Promise<ToolResponse> {
        await debuggerController.resumeDebugging()
        return "Debugging resumed"
    }

    async stepOver(): Promise<ToolResponse> {
        await debuggerController.stepOver()
        return "Stepped over"
    }

    async stepInto(): Promise<ToolResponse> {
        await debuggerController.stepInto()
        return "Stepped into"
    }

    async stepOut(): Promise<ToolResponse> {
        await debuggerController.stepOut()
        return "Stepped out"
    }

    async getCallStack(): Promise<ToolResponse> {
        const callStack = debuggerController.getCallStack()
        return JSON.stringify(callStack)
    }

    async getVariables(): Promise<ToolResponse> {
        const variables = debuggerController.getVariables()
        return JSON.stringify(variables)
    }

    async getBreakpoints(): Promise<ToolResponse> {
        const breakpoints = debuggerController.getBreakpoints()
        return breakpoints.map((bp: any) => `${bp.filePath}:${bp.lineNumber}`).join("\n")
    }

    async addBreakpoint(filePath: string, lineNumber: number): Promise<ToolResponse> {
        const loc: vscode.Location = new vscode.Location(vscode.Uri.file(filePath), new vscode.Position(lineNumber, 0))
        const breakpoint = debuggerController.addBreakpoint(loc)
        return breakpoint ? "Breakpoint added" : "Failed to add breakpoint"
    }

    async removeBreakpoint(filePath: string, lineNumber: number): Promise<ToolResponse> {
        const loc = new vscode.Location(vscode.Uri.file(filePath), new vscode.Position(lineNumber, 0))
        const breakpoints = debuggerController.getBreakpoints()
        const breakpoint = breakpoints.find((bp: any) => bp.filePath === filePath && bp.lineNumber === lineNumber)
        if (breakpoint) {
            debuggerController.removeBreakpoint(breakpoint)
        }
        return breakpoint ? "Breakpoint removed" : "Failed to remove breakpoint"
    }

    async evaluateExpression(expression: string): Promise<ToolResponse> {
        const result = debuggerController.evaluateExpression(expression)
        return result
    }
    ...
    async executeCommand(command?: string, returnEmptyStringOnSuccess: boolean = false): Promise<ToolResponse> {
        ...
        if (this.automaticallyRunTasks) {
            await this.say("text", command)
            await delay(3000) // 3-second delay

            if (this.diffViewFocused) {
                this.diffViewFocused = false // Reset the flag
  ...
  `
    ...
    async recursivelyMakeClaudeRequests(userContent: UserContent): Promise<ClaudeRequestResult> {
        ...
        await this.addToApiConversationHistory({ role: "user", content: this.providerName === 'azure' ? JSON.stringify(userContent) : userContent })
        ...
        try {
            ...
            for (const contentBlock of response.content) {
                if (contentBlock.type === "tool_use") {
                    ...
                    const toolInput = JSON.parse(contentBlock.input as string)
            ...
            if (assistantResponses.length > 0) {
                const results = assistantResponses.map((contentBlock) => {
                    if(contentBlock.type === 'tool_use') {
                        return {
                            type: 'text',
                            text: `Tool Used: ${contentBlock.name}\nTool Input: ${JSON.stringify(contentBlock.input)}`
                        }
                    } else return contentBlock;
                });
                await this.addToApiConversationHistory({ role: "assistant", content: (this.providerName === 'azure' ? results : assistantResponses) as any})
            } else {
            ...
            if (attemptCompletionBlock) {
                let result = await this.executeTool(
                    ...
                    JSON.parse(attemptCompletionBlock.input as string)
                )
                // this.say(
                // 	"tool",
  ...
  `
    ...
    async writeToFileWithConfirmation(relPath?: string, newContent?: string): Promise<ToolResponse> {
        // Existing implementation
        return this.writeToFile(relPath, newContent)
    }

    async readFileWithConfirmation(relPath?: string): Promise<ToolResponse> {
        // Existing implementation
        return this.readFile(relPath)
    }

    async listFilesWithConfirmation(relDirPath?: string, recursiveRaw?: string): Promise<ToolResponse> {
        // Existing implementation
        return this.listFiles(relDirPath, recursiveRaw)
    }

    async listCodeDefinitionNamesWithConfirmation(relDirPath?: string): Promise<ToolResponse> {
        // Existing implementation
        return this.listCodeDefinitionNames(relDirPath)
    }

    async searchFilesWithConfirmation(relDirPath: string, regex: string, filePattern?: string): Promise<ToolResponse> {
        // Existing implementation
        return this.searchFiles(relDirPath, regex, filePattern)
    }

    async executeCommandWithConfirmation(command?: string, returnEmptyStringOnSuccess: boolean = false): Promise<ToolResponse> {
        // Existing implementation
        return this.executeCommand(command, returnEmptyStringOnSuccess)
    }
